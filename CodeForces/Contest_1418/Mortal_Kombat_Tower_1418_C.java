...
public class Mortal_Kombat_Tower_1418_C {

    public static void main(String[] args) throws IOException {
        FastIO in = new FastIO(args);
        int t = in.ni();
        while (t-- > 0) {
            int n = in.ni(), a[] = in.gia(n);
            if (n == 1) {
                System.out.println(a[0]);
                continue;
            }
            int dp_my_move[] = new int[n];
            int dp_his_move[] = new int[n];
            dp_his_move[0] = a[0];
            dp_his_move[1] = a[0] + a[1];
            dp_my_move[1] = dp_his_move[0];
            if (n == 2) {
                System.out.println(Math.min(dp_his_move[n - 1], dp_my_move[n - 1]));
                continue;
            }
            dp_his_move[2] = a[0] + a[2];
            dp_my_move[2] = Math.min(dp_his_move[0], dp_his_move[1]);
            for (int i = 3; i < n; i++) {
                dp_his_move[i] = Math.min(dp_my_move[i - 1], (dp_my_move[i - 2] + a[i - 1])) + a[i];
                dp_my_move[i] = Math.min(dp_his_move[i - 1], dp_his_move[i - 2]);
            }
            System.out.println(Math.min(dp_his_move[n - 1], dp_my_move[n - 1]));
        }
        in.bw.flush();
