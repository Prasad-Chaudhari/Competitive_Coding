...
/**
 * Date: 22 Sep, 2019
 * Link: codeforces.com/problemset/problem/615/C
 * 
...
public class Running_Track_615_C {
    ...
    public static void main(String[] args) throws IOException {
        ...
        Map<Character, LinkedList<Integer>> map = new Hashtable<>();
        for (int i = 0; i < 26; i++) {
        ...
        for (int i = 0; i < m;) {
            LinkedList<Integer> l = new LinkedList<>(map.get(t[i]));
            int len = 0;
            if (l.size() == 0) {
            ...
            while (true) {
                ...
                for (int j = 0; j < size; j++) {
                    int index = l.remove();
                    if (index < 0) {
                        if (-(index + len) >= 0 && i + len < m && t[i + len] == s[-(index + len)]) {
                            p = index;
                            l.add(index);
                        }
                    } else {
                        if (index + len < n && i + len < m && t[i + len] == s[index + len]) {
                            p = index;
                            l.add(index);
                        }
                    }
            ...
            int index = p;
            if (index < 0) {
                ans.add(new Data(-index + 1, -index - len + 2));
            } else {
                ans.add(new Data(index + 1, index + len));
            }
        }
        System.out.println(ans.size());
        while (!ans.isEmpty()) {
            System.out.println(ans.peek().a + " " + ans.remove().b);
        }
        in.bw.flush();
    }
