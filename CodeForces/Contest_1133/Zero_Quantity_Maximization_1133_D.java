...
public class Zero_Quantity_Maximization_1133_D {
    ...
    public static void main(String[] args) throws IOException {
        ...
        double a[] = in.gda(n);
        double b[] = in.gda(n);
        Map<BigDecimal, Integer> map = new Hashtable<>();
        int anything = 0;
        for (int i = 0; i < n; i++) {
            if (b[i] == 0 && a[i] == 0) {
                anything++;
            } else if (a[i] != 0) {
                BigDecimal bd_a = new BigDecimal(a[i]);
                BigDecimal bd_b = new BigDecimal(b[i]);
                BigDecimal bd_d = bd_b.divide(bd_a, new MathContext(30, RoundingMode.CEILING));
                map.put(bd_d, map.getOrDefault(bd_d, 0) + 1);
            }
        }
        int max = 0;
        for (BigDecimal bd : map.keySet()) {
            max = Math.max(max, map.get(bd));
        }
        System.out.println(max + anything);
    }
