...
public class Yet_Another_Array_Restoration_1409_C {
    ...
    public static void main(String[] args) throws IOException {
        ...
        while (t-- > 0) {
            ...
            LinkedList<Integer> ansans = new LinkedList<>();
            for (int factor : factors) {
                LinkedList<Integer> ans = new LinkedList<>();
                for (int i = x; i <= y; i += factor) {
                    ans.add(i);
                }
                if (ans.size() < n) {
                    for (int i = x - factor; i > 0 && ans.size() < n; i -= factor) {
                        ans.addFirst(i);
                    }
                    for (int i = y + factor; ans.size() < n; i += factor) {
                        ans.addLast(i);
                    }
                }
                if (ans.size() == n) {
                    if (min_max > ans.peekLast()) {
                        min_max = ans.peekLast();
                        ansans = new LinkedList<>(ans);
                    }
                }
            }
