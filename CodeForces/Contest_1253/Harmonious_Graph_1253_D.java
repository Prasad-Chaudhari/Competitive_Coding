...
public class Harmonious_Graph_1253_D {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 1; i <= n; i++) {
            map.put(uf.find(i), Math.max(map.getOrDefault(uf.find(i), 0), i));
        }
        boolean b[] = new boolean[n + 1];
        int count = 0;
        for (int i = 1; i <= n; i++) {
            if (!b[i]) {
                int max = map.get(uf.find(i));
                for (int j = i; j <= max; j++) {
                    if (uf.find(i) != uf.find(j)) {
                        int mm = Math.max(map.get(uf.find(i)), map.get(uf.find(j)));
                        uf.union(i, j);
                        map.put(uf.find(i), mm);
                        max = Math.max(max, mm);
                        count++;
                    }
                    b[j] = true;
