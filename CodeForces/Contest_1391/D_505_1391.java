...
public class D_505_1391 {
    ...
    public static void main(String[] args) throws IOException {
        ...
        int best_buds[][] = new int[(int) Math.pow(2, m)][];
        ...
        long dp[][] = new long[n][(int) Math.pow(2, m)];
        for (int i = 0; i < n; i++) {
            Arrays.fill(dp[i], Integer.MAX_VALUE);
        }
        for (int i = 0; i < Math.pow(2, m); i++) {
            dp[0][i] = Integer.bitCount(int_form[0] ^ i);
        }
        for (int i = 1; i < n; i++) {
            for (int j = 0; j < Math.pow(2, m); j++) {
                for (int k : best_buds[j]) {
                    dp[i][k] = Math.min(dp[i - 1][j] + Integer.bitCount(k ^ int_form[i]), dp[i][k]);
                }
            }
        }
    ...
    private static boolean best_buds(int i, int j, int m) {
        boolean ans = true;
        int a[] = new int[m];
        int b[] = new int[m];
        for (int p = 0; p < m; p++) {
