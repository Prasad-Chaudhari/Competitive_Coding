...
public class k_Amazing_Numbers_1417_C {
    ...
    public static void main(String[] args) throws IOException {
        ...
        while (t-- > 0) {
            ...
            for (int i = 0; i <= n; i++)
                l.get(i).add(n);
            int max_gaps[] = new int[n + 1];
            for (int i = 1; i <= n; i++) {
                int gap = 0;
                int curr = l.get(i).remove();
                while (!l.get(i).isEmpty()) {
                    gap = Math.max(gap, l.get(i).peekFirst() - curr);
                    curr = l.get(i).removeFirst();
                }
                max_gaps[i] = gap;
            }
            ...
            Arrays.fill(gaps, -1);
            for (int i = 1; i <= n; i++) {
                if (max_gaps[i] != n + 1 && gaps[max_gaps[i]] == -1) {
                    gaps[max_gaps[i]] = i;
                }
            }
            for (int i = 2; i <= n; i++) {
                if (gaps[i - 1] == -1 && gaps[i] == -1) {
