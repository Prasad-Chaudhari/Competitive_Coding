...
public class a_Good_String_1385_D {
    ...
    public static void main(String[] args) throws IOException {
        ...
        while (t-- > 0) {
            int n = in.ni();
            String s = in.next();
            String a_good = make_good(s, 'a');
            int ans = 0;
            for (int i = 0; i < n; i++) {
                ans += s.charAt(i) != a_good.charAt(i) ? 1 : 0;
            }
    ...
    private static String make_good(String s, char good) {
        String ans = "";
        if (s.length() == 1) {
            return good + "";
        } else {
            int l = s.length();
            String left_change = make_good(s.substring(0, l / 2), (char) (good + 1));
            String right_change = make_good(s.substring(l / 2), (char) (good + 1));
            ...
            int lc = 0, rc = 0;
            left_change = left_change + same;
            right_change = same + right_change;
            for (int i = 0; i < l; i++) {
                lc += s.charAt(i) != left_change.charAt(i) ? 1 : 0;
                rc += s.charAt(i) != right_change.charAt(i) ? 1 : 0;
            }
            if (lc < rc) {
                ans = left_change;
            } else {
