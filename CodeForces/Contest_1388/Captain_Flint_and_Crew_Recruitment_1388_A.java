...
/**
 * Date: 30 Jul, 2020
 * Link: https://codeforces.com/contest/1388/problem/A
 * 
...
public class Captain_Flint_and_Crew_Recruitment_1388_A {

    public static void main(String[] args) throws IOException {
        FastIO in = new FastIO(args);
        int primes[] = Primes.getPrimes(200000);
        LinkedList<Integer> near_primes = new LinkedList<>();
        for (int i = 0; i < primes.length; i++) {
            for (int j = i + 1; j < primes.length; j++) {
                if (1l * primes[i] * primes[j] <= 200000) {
                    near_primes.add(primes[i] * primes[j]);
                } else {
                    break;
                }
            }
        }
        Collections.sort(near_primes);
        int a[] = new int[near_primes.size()];
        for (int i = 0; i < a.length; i++) {
            a[i] = near_primes.removeFirst();
        }
        int t = in.ni();
        outer: while (t-- > 0) {
            int n = in.ni();
                    ...
                    for (int j = i + 1; j < a.length; j++) {
                        for (int k = j + 1; k < a.length; k++) {
                            ...
                            if (p != a[i] && p != a[j] && p != a[k]) {
                                ...
                                System.out.println(a[i] + " " + a[j] + " " + a[k] + " " + p);
                                continue outer;
                            }
                        }
                    }
