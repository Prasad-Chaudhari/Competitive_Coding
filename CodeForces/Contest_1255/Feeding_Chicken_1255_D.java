...
public class Feeding_Chicken_1255_D {
    ...
    public static void main(String[] args) throws IOException {
        ...
        char a[] = new char[62];
        for (int i = 0; i < 62; i++) {
            a[i] = i < 26 ? (char) (i + 'a') : (i < 52) ? (char) (i - 26 + 'A') : (char) (i - 52 + '0');
        }
        while (t-- > 0) {
            int n = in.ni();
            int m = in.ni();
            ...
            count = 0;
            for (int i = 0; i < n; i++) {
                for (int j = (i & 1) == 0 ? 0 : m - 1; (i & 1) == 0 ? j < m : j >= 0; j = ((i & 1) == 0) ? ++j : --j) {
                    if (c[i][j] == 'R') {
                        count++;
                    }
                    ans[i][j] = a[index];
            ...
            for (int i = 0; i < n; i++) {
                for (int j = 0; j < m; j++)
                    if (index == k && index < 62 && ans[i][j] == a[index])
                        ans[i][j] = a[index - 1];
                in.println(new String(ans[i]));
            }
        }
        in.bw.flush();
    }
