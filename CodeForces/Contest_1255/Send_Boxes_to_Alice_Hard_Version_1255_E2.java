...
/**
 * Date: 19 Nov, 2019
 * Link: https://codeforces.com/contest/1255/problem/E2
 * 
...
public class Send_Boxes_to_Alice_Hard_Version_1255_E2 {
    static class Primes {
    ...
    public static void main(String[] args) throws IOException {
        ...
        int primes[] = Primes.getPrimes(1000000);
        int n = in.ni();
        int a[] = in.gia(n);
        long total = 0;
        for (int i : a)
        ...
        for (long prime : s) {
            ...
            sum[0] = a[0];
            for (int i = 0; i < n; i++) {
                long r = Math.min(sum[i] % prime, prime - sum[i] % prime);
                ans += r;
                if (i < n - 1)
                    sum[i + 1] = sum[i] + b[i + 1];
            }
