...
public class League_of_Leesins_1255_C {
    ...
    public static void main(String[] args) throws IOException {
        ...
        ArrayList<LinkedList<Integer>> pos = new ArrayList<>();
        for (int i = 0; i <= n; i++) {
        ...
        Collections.sort(q.get(pos.get(one).peek()), new Comparator<Integer>() {
            @Override
            public int compare(Integer a, Integer b) {
                return Integer.compare(pos.get(a).size(), pos.get(b).size());
            }
        });
        ...
        for (int i = 0; i < n; i++) {
            ...
            PriorityQueue<LinkedList<Integer>> pq2 = new PriorityQueue<>(c);
            ans.add(p.remove());
            for (int poss : pos.get(ans.peekLast())) {
                q.get(poss).remove(ans.peekLast());
                pq2.add(q.get(poss));
            }
            while (!pq.isEmpty()) {
                pq2.add(pq.remove());
