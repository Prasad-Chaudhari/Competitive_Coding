...
public class Kamil_and_Making_a_Stream_1230_E {
    ...
    public static void main(String[] args) throws IOException {
        ...
        ArrayList<Map<Long, Long>> al = new ArrayList<>();
        for (int i = 0; i <= n; i++) {
        ...
        while (!st.isEmpty()) {
            int root = st.pop();
            for (long key : al.get(root).keySet()) {
                ans += key % mod * al.get(root).get(key);
            }
            ans %= mod;
            for (int i : adja[root]) {
                if (!b[i]) {
                    ...
                    for (long key : al.get(root).keySet()) {
                        ...
                        if (key == 0 && x[i - 1] == 0) {
                            ...
                            gcd = g.calc_gcd(key, x[i - 1]);
                        }
                        map.put(gcd, map.getOrDefault(gcd, 0l) + al.get(root).get(key));
                    }
                    map.put(x[i - 1], map.getOrDefault(x[i - 1], 0l) + 1);
                }
            }
        }
