...
public class Acacius_and_String_1379_A {
    ...
    public static void main(String[] args) throws IOException {
        ...
        outer: while (t-- > 0) {
            ...
            for (int i = 0; i + a.length() <= n; i++) {
                String s2 = s.substring(0, i) + a + s.substring(i + a.length());
                boolean p = true;
                for (int j = 0; j < n; j++) {
                    if (s.charAt(j) != '?' && s2.charAt(j) != s.charAt(j)) {
                        p = false;
                    }
                }
                if (p) {
                    ...
                    for (int j = 0; j + a.length() <= n; j++) {
                        if (a.equals(s2.substring(j, j + a.length()))) {
                            pp++;
                        }
                    }
                    if (pp == 1) {
