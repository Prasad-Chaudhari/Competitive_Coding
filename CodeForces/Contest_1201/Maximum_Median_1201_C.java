...
public class Maximum_Median_1201_C {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 0; i < map.size(); i++) {
            if (size + d[i].b > n / 2) {
                curr_median = d[i].a;
                start = size + d[i].b - n / 2;
                index = i;
                break;
            }
            size += d[i].b;
        }
        while (k > 0 && index < map.size() - 1) {
            long diff = d[index + 1].a - d[index].a;
            if (k >= 1l * diff * start) {
                curr_median = d[index + 1].a;
                k -= 1l * diff * start;
                start += d[index + 1].b;
                index++;
            } else {
        ...
        curr_median += k / ((n / 2) + 1);
        System.out.println(curr_median);
        in.bw.flush();
    }
