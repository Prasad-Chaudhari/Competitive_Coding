...
public class BerOS_File_Suggestion_1070_H {
    ...
    public static void main(String[] args) throws IOException {
        ...
        Map<String, String> file = new Hashtable<>();
        for (int i = 0; i < n; i++) {
            String s = in.next();
            for (int j = 0; j < s.length(); j++) {
                for (int k = j + 1; k <= s.length(); k++) {
                    String sub = s.substring(j, k);
                    if (!file.containsKey(sub)) {
                        freq.put(sub, freq.getOrDefault(sub, 0) + 1);
                    } else {
                        if (!file.get(sub).equals(s)) {
                            freq.put(sub, freq.getOrDefault(sub, 0) + 1);
                        }
                    }
                    file.put(sub, s);
        ...
        while (q-- > 0) {
            ...
            if (freq.getOrDefault(s, 0) == 0) {
                ...
                System.out.println(freq.get(s) + " " + file.get(s));
            }
        }
    }
