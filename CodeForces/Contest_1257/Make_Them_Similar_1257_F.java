...
public class Make_Them_Similar_1257_F {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (x = 0; x < 1 << 15; ++x) {
            curr = node;
            for (i = 0; i < n; ++i) {
                l = Integer.bitCount(b[i] ^ x);
                if (!curr.map.containsKey(l)) {
                    curr.map.put(l, new Node());
                }
                curr = curr.map.get(l);
                curr.value = x;
            }
        ...
        for (x = 0; x < 1 << 15; ++x) {
            i = 0;
            while (i < n) {
                list[i] = (Integer.bitCount(b[++i - 1] ^ x));
            }
            t = 0;
            while (t <= 30) {
                curr = node;
                i = 0;
                while (i < n && curr != null) {
                    curr = curr.map.getOrDefault(t - list[++i - 1], null);
                }
                if (curr != null) {
                    System.out.println((x << 15) + curr.value);
                    return;
                }
                ++t;
            }
