...
public class Valeriy_and_Deque_1180_C {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 0; i < n; i++) {
            int fi = l.removeFirst();
            int sec = l.removeFirst();
            d[k++] = new Data(fi, sec);
            if (fi > sec) {
                l.addFirst(fi);
                l.addLast(sec);
        ...
        while (q-- > 0) {
            long m = in.nl();
            if (m <= n) {
                in.println(d[(int) m - 1].a + " " + d[(int) m - 1].b);
            } else {
                long diff = m - n;
                int index = (int) (diff % (n - 1));
                if (index == 0) {
                    index += n - 1;
                }
                in.println(max + " " + a[index]);
            }
        }
        in.bw.flush();
    }
