...
public class Boats_Competition_1399_C {
    ...
    public static void main(String[] args) throws IOException {
        ...
        while (t-- > 0) {
            ...
            int w[] = in.gia(n);
            int max = -1;
            for (int combined_weight = 1; combined_weight <= 2 * n; combined_weight++) {
                boolean b[] = new boolean[n];
                int participants = 0;
                for (int i = 0; i < n; i++) {
                    for (int j = i + 1; j < n; j++) {
                        if (!b[i] && !b[j]) {
                            if (w[i] + w[j] == combined_weight) {
                                participants += 2;
                                b[i] = true;
                                b[j] = true;
                            }
