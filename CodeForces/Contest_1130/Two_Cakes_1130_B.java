...
public class Two_Cakes_1130_B {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 2 * n - 2; i >= 0; i--) {
            if (Math.abs(sa - d[i].b) + Math.abs(di - d[i + 1].b) < Math.abs(sa - d[i + 1].b) + Math.abs(di - d[i].b)) {
                ans += Math.abs(sa - d[i].b) + Math.abs(di - d[i + 1].b);
                sa = d[i].b;
                di = d[i + 1].b;
            } else {
                ans += Math.abs(di - d[i].b) + Math.abs(sa - d[i + 1].b);
                di = d[i].b;
                sa = d[i + 1].b;
            }
            i--;
        ...
        for (int i = 2 * n - 2; i >= 0; i--) {
            if (Math.abs(sa - d[i].b) + Math.abs(di - d[i + 1].b) < Math.abs(sa - d[i + 1].b) + Math.abs(di - d[i].b)) {
                ans2 += Math.abs(sa - d[i].b) + Math.abs(di - d[i + 1].b);
                sa = d[i].b;
                di = d[i + 1].b;
            } else {
                ans2 += Math.abs(di - d[i].b) + Math.abs(sa - d[i + 1].b);
                di = d[i].b;
                sa = d[i + 1].b;
            }
            i--;
        }
        System.out.println(Math.min(ans + sa + di, ans2 + sa + di));
