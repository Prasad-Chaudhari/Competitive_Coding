...
public class Enemy_is_weak_61_E {
    ...
    public static void main(String[] args) throws IOException {
        ...
        segtree.getArray(new int[n]);
        long ans[] = new long[n];
        long ans2[] = new long[n];
        for (int i = 0; i < n; i++) {
            segtree.update(0, n - 1, 1, a[i], 1);
            if (a[i] != n - 1) {
                ans[i] = segtree.query(0, n - 1, 1, a[i] + 1, n - 1);
            }
        }
        SegmentTree segtree2 = new SegmentTree();
        segtree2.getArray(new int[n]);
        for (int i = n - 1; i >= 0; i--) {
            segtree2.update(0, n - 1, 1, a[i], 1);
            if (a[i] != 0) {
                ans2[i] = segtree2.query(0, n - 1, 1, 0, a[i] - 1);
            }
        }
        long sum = 0;
        for (int i = 0; i < n; i++) {
