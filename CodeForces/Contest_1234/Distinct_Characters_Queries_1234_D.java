...
public class Distinct_Characters_Queries_1234_D {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 0; i < 26; i++) {
            seg[i] = new SegmentTree();
            seg[i].getArray(new int[n]);
        }
        for (int i = 0; i < n; i++) {
            seg[c[i] - 'a'].update(0, n - 1, 1, i, 1);
        }
        int q = in.ni();
        while (q-- > 0) {
            if (in.ni() == 1) {
                ...
                int r = in.ni() - 1;
                for (int i = 0; i < 26; i++) {
                    count += seg[i].query(0, n - 1, 1, l, r) == 0 ? 0 : 1;
                }
                in.println(count);
            }
        }
