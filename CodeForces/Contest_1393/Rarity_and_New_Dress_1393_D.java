...
/**
 * Date: 19 Sep, 2020
 * Link: https://codeforces.com/problemset/problem/1393/D
 * 
...
public class Rarity_and_New_Dress_1393_D {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < m; j++) {
                ...
                if (i == 0 || i == n - 1 || j == 0 || j == m - 1)
                    ...
                    if (c[i - 1][j] == c[i][j] && c[i + 1][j] == c[i][j] && c[i][j - 1] == c[i][j]
                            && c[i][j + 1] == c[i][j]) {
                        b[i][j] = false;
                    } else {
                        iter[end++] = (i * m + j);
        ...
        while (start != end) {
            ...
            int x = r / m;
            int y = r % m;
            if (x > 0 && c[x - 1][y] == c[x][y] && !b[x - 1][y]) {
                b[x - 1][y] = true;
                a[x - 1][y] = a[x][y] + 1;
                iter[end++] = ((x - 1) * m + y);
            }
            if (x < n - 1 && c[x + 1][y] == c[x][y] && !b[x + 1][y]) {
                b[x + 1][y] = true;
                a[x + 1][y] = a[x][y] + 1;
                iter[end++] = ((x + 1) * m + y);
            }
            if (y > 0 && c[x][y - 1] == c[x][y] && !b[x][y - 1]) {
                b[x][y - 1] = true;
                a[x][y - 1] = a[x][y] + 1;
                iter[end++] = (x * m + y - 1);
            }
            if (y < m - 1 && c[x][y + 1] == c[x][y] && !b[x][y + 1]) {
                b[x][y + 1] = true;
                a[x][y + 1] = a[x][y] + 1;
                iter[end++] = (x * m + y + 1);
            }
