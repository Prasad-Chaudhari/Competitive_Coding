...
public class Boboniu_and_Bit_Operations_1395_C {
    ...
    public static void main(String[] args) throws IOException {
        ...
        int a[] = in.gia(n);
        int b[] = in.gia(m);
        for (int ans = 0; ans < Math.pow(2, 9); ans++) {
            boolean p = true;
            for (int i = 0; i < n; i++) {
                boolean q = false;
                for (int j = 0; j < m; j++) {
...
// boolean c_contributors[][] = new boolean[9][n];
// int c_count_at_bit[] = new int[9];
// int choice_of_a[] = new int[n];
// for (int i = 0; i < n; i++) {
// int k = a_full[i] & b_full[0];
// for (int j = 0; j < 9; j++) {
// c_count_at_bit[j] += k & 1;
// if ((k & 1) == 1) {
// c_contributors[j][i] = true;
// }
// k >>= 1;
// }
// choice_of_a[i] = 0;
...
// int count = 0;
// for (int j = 0; j < n; j++) {
// if (c_contributors[bit][j]) {
// count += ((a_full[j] >> bit) & 1) & ((b_full[i] >> bit) & 1);
// }
// }
// if (count == 0) {
// for (int j = 0; j < n; j++) {
// if (c_contributors[bit][j]) {
// count -= ((a_full[j] >> bit) & 1) & ((b_full[choice_of_a[j]] >> bit) & 1);
// c_contributors[bit][j] = false;
// }
// }
