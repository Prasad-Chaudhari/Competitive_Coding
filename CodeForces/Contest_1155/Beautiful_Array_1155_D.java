...
/**
 * Date: 23 Sep, 2019
 * Link: http://codeforces.com/problemset/problem/1155/D
 * 
...
public class Beautiful_Array_1155_D {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 0; i < n; i++) {
            ...
            left[i] = Math.max(0, sum[i] - min);
        }
        long suf_sum[] = new long[n];
        suf_sum[n - 1] = a[n - 1];
        for (int i = n - 2; i >= 0; i--) {
        ...
        min = 0;
        for (int i = n - 1; i >= 0; i--) {
            min = Math.min(min, suf_sum[i]);
            right[i] = Math.max(0, suf_sum[i] - min);
        }
        long max = Long.MIN_VALUE;
        long best_l = Long.MIN_VALUE;
        for (int i = 0; i < n; i++) {
            long p = x * sum[i];
            if (i + 1 < n) {
                p += right[i + 1];
            ...
            best_l = Math.max(best_l, q);
            max = Math.max(p + best_l, max);
            max = Math.max(max, left[i]);
            // System.out.println(p + " " + best_l + " " + left[i]);
        }
