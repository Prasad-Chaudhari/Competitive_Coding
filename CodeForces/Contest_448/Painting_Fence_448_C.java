...
public class Painting_Fence_448_C {
    ...
    public static long ans(int a[], int l, int r) {
        if (l > r) {
            return 0;
        }
        if (l == r) {
        ...
        long ans2 = min;
        LinkedList<Integer> non_zero = new LinkedList<>();
        for (int i = l; i <= r; i++) {
            if (a[i] != 0) {
                non_zero.add(i);
            }
        ...
        if (!non_zero.isEmpty()) {
            int start = non_zero.remove();
            int end = start;
            while (!non_zero.isEmpty()) {
                if (end + 1 != non_zero.peek()) {
                    ans2 += ans(a, start, end);
                    start = non_zero.remove();
                    end = start;
                } else {
                    end = non_zero.remove();
                }
