...
public class Bad_Luck_Island_540_D {
    ...
    public static void main(String[] args) throws IOException {
        ...
        int s = in.ni();
        int p = in.ni();
        double fac[] = new double[r + p + s + 1];
        fac[0] = 1;
        for (int i = 1; i < fac.length; i++) {
            fac[i] = i * fac[i - 1];
        }
        double pro[][][] = new double[r + 1][s + 1][p + 1];
        pro[r][s][p] = 1;
        for (int i = r; i >= 0; i--) {
            for (int j = s; j >= 0; j--) {
                for (int k = p; k >= 0; k--) {
