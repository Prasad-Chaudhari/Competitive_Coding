...
/**
 * Date: 3 Nov, 2018
 * Link: https://www.codechef.com/NOV18B/problems/HMAPPY1
 *
...
public class HMAPPY1 {
    ...
    public static void main(String[] args) throws IOException {
        ...
        int index = max - 1;
        lengths[index] = a[max - 1];
        for (int i = max - 2; i >= max - n; i--) {
            if (a[i] == a[i + 1]) {
                if (a[i] == 1) {
                    lengths[index]++;
                }
        ...
        int right = max - 1;
        int left = index;
        MaxSegmentTree maxsegtree = new MaxSegmentTree();
        maxsegtree.getArray(lengths);
        maxsegtree.build(0, max - 1, 1);
        char c[] = in.next().toCharArray();
        index = max - 1 - n;
        for (char cc : c) {
            if (cc == '?') {
                int ans = maxsegtree.query(0, max - 1, 1, left, right);
                System.out.println(ans > k ? k : ans);
            } else {
                if (a[index] == 1) {
                    if (a[index + n - 1] == 0) {
                        right--;
                        ...
                        lengths[right]--;
                        maxsegtree.update(0, max - 1, 1, right, lengths[right]);
                    }
                    if (a[index + 1] == 0) {
                        left--;
                        lengths[left]++;
                        maxsegtree.update(0, max - 1, 1, left, lengths[left]);
                    } else {
                        lengths[left]++;
                        maxsegtree.update(0, max - 1, 1, left, lengths[left]);
                    }
